...
def generate(
    ...
    for resource in spec.resources:
        ...
        tf_file = out_dir / f"{resource.name}.tf"
        tf_file.write_text(tf_content)
        console.print(f"  [green]✓[/green] {resource.name}.tf [dim]({resource.type})[/dim]")
...
def _write_provider(region: str, out_dir: Path) -> None:
    ...
    (out_dir / "provider.tf").write_text(content)
    console.print(f"  [green]✓[/green] provider.tf")
